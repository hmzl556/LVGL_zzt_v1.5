...
{
	(void)t;                                             /* 未使用定时器指针 */
	static uint32_t s_clock_seconds = 16u * 3600u + 30u * 60u; /* 仿真时钟起点 16:30:00 */
...
/* 构建报警页：顶部栏返回/启停/电源 + 状态栏；中部缺液告警；底栏 bar_01 常驻 */
...
static void program_admin_init_factory(void)
{
	static const ui_program_admin_t factory[TOTAL_PROGRAMS] = {
		/* 0 大物：金额9 | 洗12min 漂10min×2 脱9min 总41min | 800rpm | 40℃ | 水位智能 */
		{ 9, 12*60, 11*60, 8*60,  2, 800,  2,  true,
		...
		/* 1 单脱水：金额5 | 无洗涤/漂洗 | 脱9min 总9min | 800rpm | 水温-- | 水位-- */
		{ 5,     0,     0, 8*60, -1, 800, -1, false,
		  PROG_CAP_PRICE | PROG_CAP_SPIN_DUR | PROG_CAP_SPIN_RPM },
		/* 2 标准洗：金额8 | 洗10min 漂8min×2 脱9min 总35min | 800rpm | 30℃ | 水位智能 */
		{ 8, 10*60,  8*60, 6*60,  2, 800,  1,  true,
		  PROG_CAP_PRICE | PROG_CAP_WASH | PROG_CAP_RINSE_DUR | PROG_CAP_SPIN_DUR |
		  PROG_CAP_RINSE_CNT | PROG_CAP_SPIN_RPM | PROG_CAP_TEMP | PROG_CAP_WATER },
		/* 3 筒自洁：金额1 | 无洗涤 | 漂2min×1 脱1min 总3min | 600rpm | 水温-- | 水位-- */
		{ 1,     0,  3*60,   10, -1, 600, -1, false,
		  PROG_CAP_PRICE | PROG_CAP_RINSE_DUR | PROG_CAP_SPIN_DUR | PROG_CAP_RINSE_CNT | PROG_CAP_SPIN_RPM },
		/* 4 快洗：金额7 | 洗9min 漂8min×1 脱7min 总24min | 800rpm | COLD | 水位智能 */
		{ 7,  9*60,  8*60, 8*60,  1, 800,  0,  true,
		  PROG_CAP_PRICE | PROG_CAP_WASH | PROG_CAP_RINSE_DUR | PROG_CAP_SPIN_DUR |
		  PROG_CAP_RINSE_CNT | PROG_CAP_SPIN_RPM | PROG_CAP_TEMP | PROG_CAP_WATER },
	};
	for(int i = 0; i < TOTAL_PROGRAMS; i++) {
...
static void program_admin_clamp_cfg(ui_program_admin_t * c)
{
	...
	if(c->wash_min > 90*60) c->wash_min = 90*60;
	if(c->rinse_min > 90*60) c->rinse_min = 90*60;
	if(c->spin_min > 90*60) c->spin_min = 90*60;
...
static void build_pay(void)
{
		...
		lv_obj_set_width(g_lbl_pay_hint, 560);                       /* 限制宽度以自动折行 */
