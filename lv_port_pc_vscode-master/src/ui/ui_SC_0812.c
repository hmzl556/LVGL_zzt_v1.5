...
static void ui_screen_load(lv_obj_t * scr)
{
	...
	if(scr == g_scr_off) {
		...
		//单次追加时间
		param_change[3] = g_prog_cfg[g_wheel_sel].add_time_min <= 0 ? 0 : g_prog_cfg[g_wheel_sel].add_time_min * 60;
		//加热时间
		param_change[4] = g_prog_cfg[g_wheel_sel].init_dry_min * 60;
		//冷风时间
		param_change[5] = g_prog_cfg[g_wheel_sel].cool_min * 60;
		//防缠绕
...
/* 当前屏幕不进入空闲待机（待机页、运行页、烘干完成页、报警弹层可见、管理员页常亮） */
