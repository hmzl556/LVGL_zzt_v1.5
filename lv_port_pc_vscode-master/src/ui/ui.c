...
static bool g_ui_payment_wechat_enabled = false;
...
#define HOME_CAROUSEL_LONG_PRESS_MS 800u  /* 主页轮播编码器长按：固定居中并回启停 */
...
static void admin_sound_back_to_menu1(void);  //离开声音控制页：回 menu1
...
static lv_obj_t * g_scr_alarm;			//报警页
...
static lv_timer_t * g_home_pay_load_timer;     /* 延迟进支付页，避免同一次确认连带进支付完成 */
...
static lv_obj_t * g_alarm_img_warn;
static lv_obj_t * g_alarm_lbl_hint;
static lv_obj_t * g_alarm_img_bar;
...
static lv_obj_t * g_home_carousel_enc;           /* 轮播区编码器焦点代理（旋转选程序，长按回启停） */
static bool g_home_carousel_exit_suppress_click; /* 轮播长按回启停后，吞掉同一次松手的 CLICKED */
static bool g_home_carousel_long_latched;        /* 同一次按下只处理一次 LONG_PRESSED */
static lv_timer_t * g_home_carousel_suppress_timer;
...
/* 管理员「程序设置」扩展参数（表3.1 初值；PC 演示时长单位为秒） */
...
typedef struct {
    ...
    uint16_t wash_min;      /* 单次洗涤时长（秒）；无洗涤时忽略 */
    uint16_t rinse_min;     /* 单次漂洗时长（秒） */
    uint16_t spin_min;      /* 脱水时长（秒） */
...
static void home_carousel_encoder_lock_and_exit(void);  //轮播固定居中并焦点回启停
static void cb_home_carousel_encoder(lv_event_t * e);  //轮播编码器：旋转循环选程序，长按回启停
static void cb_home_runpause(lv_event_t * e);  //主页启停：隔离轮播长按松手误触
...
static void build_alarm(void);  //构建报警页：顶栏、液位告警图/文案、底栏
static void alarm_content_update(void);  //按液位检测结果刷新报警页中部图与文字
...
// 主页编码器 focus 顺序：启停→电源→语言→管理员→轮播；不首尾循环，轮播仅长按回启停
...
/* 轮播长按回启停后 400ms：解除吞掉 CLICKED 的标志 */
static void home_carousel_suppress_timer_cb(lv_timer_t * t)
{
    (void)t;
    g_home_carousel_suppress_timer = NULL;
    g_home_carousel_exit_suppress_click = false;
}
...
static void home_carousel_encoder_lock_and_exit(void)
{
    ...
    /* 长按过程中已切到启停：松手时 LVGL 仍可能对启停发 CLICKED，需隔离 */
    g_home_carousel_exit_suppress_click = true;
    if(g_home_carousel_suppress_timer != NULL) {
        lv_timer_delete(g_home_carousel_suppress_timer);
    }
    g_home_carousel_suppress_timer = lv_timer_create(home_carousel_suppress_timer_cb, 400, NULL);
    lv_timer_set_repeat_count(g_home_carousel_suppress_timer, 1);
...
/* 主页启停：吞掉轮播长按固定后同一次松手的 CLICKED，避免误进支付等页 */
static void cb_home_runpause(lv_event_t * e)
{
    ...
    if(code == LV_EVENT_CLICKED) {
        if(g_home_carousel_exit_suppress_click) {
            g_home_carousel_exit_suppress_click = false;
            if(g_home_carousel_suppress_timer != NULL) {
                lv_timer_delete(g_home_carousel_suppress_timer);
                g_home_carousel_suppress_timer = NULL;
...
/* 轮播编码器：编辑模式旋转选程序；LONG_PRESSED(800ms) 固定居中并回启停 */
static void cb_home_carousel_encoder(lv_event_t * e)
{
    ...
    if(code == LV_EVENT_FOCUSED) {
        ui_indev_set_encoder_long_press_ms(HOME_CAROUSEL_LONG_PRESS_MS);
        if(g_group_home != NULL) {
            lv_group_set_editing(g_group_home, true);
        }
        return;
    ...
    if(code == LV_EVENT_DEFOCUSED) {
        ui_indev_set_encoder_long_press_ms(CHILD_LOCK_LONG_PRESS_MS);
    ...
    if(code == LV_EVENT_PRESSED) {
        g_home_carousel_long_latched = false;
        return;
    }
    ...
    if(code == LV_EVENT_LONG_PRESSED) {
        if(!g_home_carousel_long_latched) {
            g_home_carousel_long_latched = true;
...
/* 延迟 1 tick 进入支付页，避免同一次确认误进支付完成 */
static void home_enter_pay_deferred_cb(lv_timer_t * t)
{
    lv_timer_delete(t);
    g_home_pay_load_timer = NULL;
    if(g_ui_child_lock) return;
    if(g_wheel_sel < 0 || g_wheel_sel >= TOTAL_PROGRAMS) return;
    g_pay_focus_back_on_enter = true;
    ui_screen_load(g_scr_pay);
}

// 延迟进入支付页（含二维码），避免同一次点击/确认连带触发支付页「启停」进支付完成
static void home_enter_pay_screen_request(void)
{
    if(g_ui_child_lock) return;
    if(g_wheel_sel < 0 || g_wheel_sel >= TOTAL_PROGRAMS) return;
    if(g_home_pay_load_timer != NULL) return;
    g_home_pay_load_timer = lv_timer_create(home_enter_pay_deferred_cb, 1, NULL);
    lv_timer_set_repeat_count(g_home_pay_load_timer, 1);
}
...
//编码器 group 添加函数：程序设置页面：textarea/roller 获焦时在白框内侧显示橙色 border
static void ui_encoder_group_add_prog_field(lv_group_t * group, lv_obj_t * obj)
{
    if(group == NULL || obj == NULL) return;
    lv_obj_add_flag(obj, LV_OBJ_FLAG_CLICK_FOCUSABLE);
    style_prog_field_encoder_focus_inner(obj);//内侧显示橙色 border
    lv_group_add_obj(group, obj);
}
...
static void create_screens(void) //创建界面，并按当前显示分辨率设满屏宽高
{
    ...
    g_scr_alarm = lv_obj_create(NULL);
    ...
    style_screen_base(g_scr_alarm);                //报警
    ...
    lv_obj_set_size(g_scr_alarm, UI_FIXED_W, UI_FIXED_H);
...
static void ui_screen_load(lv_obj_t * scr)
{
    ...
    if(scr == g_scr_off) {
        ...
        pay_done_timer_start();                          /* 2s 后进运行页 */
    }
    else if(scr == g_scr_alarm) {
        ui_set_encoder_group(g_group_alarm);
...
/* 当前屏幕不进入空闲待机（待机页、运行页、洗涤完成页、报警页常亮） */
static bool ui_idle_screen_keeps_awake(lv_obj_t * scr)
{
    if(scr == NULL) return false;
    return scr == g_scr_off || scr == g_scr_running || scr == g_scr_end || scr == g_scr_alarm || scr == g_scr_admin;
}
...
//创建空闲计时器并注册输入监听（周期由 g_ui_dormancy_timeout_ms / 管理员「待机时间」设置）
...
/* 仅编码器：轮播获焦时 800ms 触发 LONG_PRESSED，失焦后恢复童锁 3s */
...
static void build_home(void)
{
        ...
        lv_obj_t * btn_runpause = add_encoder_top_btn(top, "启停", 100, NULL); /* 固定中文：启停 */
        g_home_btn_runpause = btn_runpause;
        lv_obj_add_event_cb(btn_runpause, cb_home_runpause, LV_EVENT_ALL, g_scr_pay); /* 编码器/触摸进支付；轮播长按回启停时隔离误触 */
        g_home_btn_power = add_encoder_top_btn(top, "电源", 180, NULL); /* 固定中文：电源 */
        lv_obj_add_event_cb(g_home_btn_power, cb_load_screen, LV_EVENT_CLICKED, g_scr_alarm); /* 临时：进报警页 */
        ...
        for(int i = 0; i < CAROUSEL_VISIBLE_SLOTS; i++) {
                ...
                ui_set_obj_font(lab, s_font_sc_30);        /* 30 号中文字体（含英文 glyph） */
        ...
        lv_obj_align(lbl_time, LV_ALIGN_CENTER, bar_col1_x, 0); /* 第 1 列水平中心 */
...
static void running_screen_sync_mode_name(void)
{
    ...
    if(g_running_mid != NULL) {
        lv_obj_update_layout(g_running_mid);           /* 程序名变长/变短后 mid 框宽度变化 */
    }
...
static void build_running(void)
{
        ...
        lv_label_set_text(time_txt, "0:24");                 /* 占位初值，进入运行页后按程序时长重算 */
...
//PC仿真缺液，实机需根据情况删掉这段代码
/* 液体不足判定阈值（0=空，100=满；实际硬件在检测函数内读传感器） */
#define UI_FLUID_LEVEL_LOW_THRESHOLD  20u

//缺液原因枚举
typedef enum {
    UI_ALARM_REASON_NONE = 0,
    UI_ALARM_REASON_SOFTENER_LOW,
    UI_ALARM_REASON_DETERGENT_LOW,
} ui_alarm_reason_t;

/* PC 仿真液位；量产时改为寄存器 读取 */
static uint8_t g_fluid_softener_pct = 50u;	//柔顺剂液位
static uint8_t g_fluid_detergent_pct = 15u;	//洗涤剂液位

//柔顺剂不足
static bool ui_is_softener_low(void)
{
    return g_fluid_softener_pct < UI_FLUID_LEVEL_LOW_THRESHOLD;
}

//洗涤剂不足
static bool ui_is_detergent_low(void)
{
    return g_fluid_detergent_pct < UI_FLUID_LEVEL_LOW_THRESHOLD;
}

//获取缺液原因
static ui_alarm_reason_t ui_alarm_reason_get(void)
{
    if(ui_is_softener_low()) {
        return UI_ALARM_REASON_SOFTENER_LOW;
    }
    if(ui_is_detergent_low()) {
        return UI_ALARM_REASON_DETERGENT_LOW;
    }
    return UI_ALARM_REASON_NONE;
}

//按缺液检测结果刷新报警页中部图与文字（文案走 i18n 表）
static void alarm_content_update(void)
{
    if(g_alarm_img_warn == NULL || g_alarm_lbl_hint == NULL) {
        return;
    }

    ui_alarm_reason_t reason = ui_alarm_reason_get();
    switch(reason) {
    case UI_ALARM_REASON_SOFTENER_LOW:
        lv_image_set_src(g_alarm_img_warn, &lack_of_softener);
        lv_label_set_text(g_alarm_lbl_hint, ui_translation(STR_ALARM_SOFTENER_LOW));
        lv_obj_remove_flag(g_alarm_img_warn, LV_OBJ_FLAG_HIDDEN);
        lv_obj_remove_flag(g_alarm_lbl_hint, LV_OBJ_FLAG_HIDDEN);
        break;
    case UI_ALARM_REASON_DETERGENT_LOW:
        lv_image_set_src(g_alarm_img_warn, &lack_of_detergent);
        lv_label_set_text(g_alarm_lbl_hint, ui_translation(STR_ALARM_DETERGENT_LOW));
        lv_obj_remove_flag(g_alarm_img_warn, LV_OBJ_FLAG_HIDDEN);
        lv_obj_remove_flag(g_alarm_lbl_hint, LV_OBJ_FLAG_HIDDEN);
        break;
    default:
        lv_obj_add_flag(g_alarm_img_warn, LV_OBJ_FLAG_HIDDEN);
        lv_obj_add_flag(g_alarm_lbl_hint, LV_OBJ_FLAG_HIDDEN);
        break;
    }
}


/* 构建报警页：顶栏返回/启停/电源 + 状态栏；中部缺液告警；底栏 bar_01 常驻 */
static void build_alarm(void)
{
        lv_obj_t * root = lv_obj_create(g_scr_alarm);
        lv_obj_set_size(root, LV_PCT(100), LV_PCT(100));
        lv_obj_set_pos(root, 0, 0);
        lv_obj_set_style_bg_opa(root, LV_OPA_TRANSP, LV_PART_MAIN);
        lv_obj_set_style_border_width(root, 0, LV_PART_MAIN);
        ...
        lv_obj_t * top = create_top_bar(root, &g_lbl_clock_alarm, NULL, NULL, NULL);
        ...
        lv_obj_add_event_cb(btn_back, cb_load_screen, LV_EVENT_CLICKED, g_scr_home); /* 临时：返回主页 */
        ui_encoder_group_add(g_group_alarm, btn_back);

        add_encoder_top_btn(top, "启停", 100, g_group_alarm);
        add_encoder_top_btn(top, "电源", 180, g_group_alarm);

        g_alarm_img_bar = lv_image_create(root);//底栏
        lv_image_set_src(g_alarm_img_bar, &bar_01);
        lv_obj_align(g_alarm_img_bar, LV_ALIGN_BOTTOM_MID, 0, -80);

        lv_obj_t * body = lv_obj_create(root);//中部区域
        lv_obj_set_size(body, LV_PCT(100), (lv_coord_t)UI_FIXED_H - 120);
        lv_obj_align(body, LV_ALIGN_TOP_MID, 0, (lv_coord_t)(UI_FIXED_H * 10 / 100));
        lv_obj_set_style_bg_opa(body, LV_OPA_TRANSP, LV_PART_MAIN);
        lv_obj_set_style_border_width(body, 0, LV_PART_MAIN);
        lv_obj_set_style_pad_all(body, 0, LV_PART_MAIN);
        lv_obj_set_style_pad_row(body, 20, LV_PART_MAIN);//上下间距
        lv_obj_set_flex_flow(body, LV_FLEX_FLOW_COLUMN);
        lv_obj_set_flex_align(body, LV_FLEX_ALIGN_CENTER, LV_FLEX_ALIGN_CENTER, LV_FLEX_ALIGN_CENTER);

        g_alarm_img_warn = lv_image_create(body);//缺液提示图
        g_alarm_lbl_hint = lv_label_create(body);//缺液提示文字
        ...
        alarm_content_update();//按缺液检测结果刷新报警页中部图与文字
}
    ...
    uint32_t * wash_out, uint32_t * rinse_total_out, uint32_t * spin_out)
{
    ...
    if((c->cap & PROG_CAP_RINSE_DUR) && (c->cap & PROG_CAP_RINSE_CNT) && c->rinse_cnt >= 0) {
        rinse_total = (uint32_t)c->rinse_min * (uint32_t)c->rinse_cnt;
    }
...
/* 写入表3.1 程序初值到 g_prog_cfg_factory[]，并复制到 g_prog_cfg[]。
 * 下标与 g_program_imgs[] / 程序设置上栏按钮一致：0=大物 1=单脱水 2=标准洗 3=筒自洁 4=快洗
 * 结构体字段：price, wash_min, rinse_min, spin_min, rinse_cnt, spin_rpm, temp_idx, water_smart, cap
 * cap 决定设置页是否可编辑；无 cap 的项 UI 显示 --。
 * 确认后 program_admin_apply_one() 同步到 g_program_profiles（主页时间/温度/金额）与 g_program_run_stages（运行阶段）。 */
static void program_admin_init_factory(void)
{
    static const ui_program_admin_t factory[TOTAL_PROGRAMS] = {
        /* 0 大物：金额9 | 洗12s 漂10s×2 脱9s 总41s | 800rpm | 40℃ | 水位智能 */
        { 9, 12, 10, 9, 2, 800, 2, true,
...
/* 将当前程序 g_prog_cfg 加载到程序设置 UI 控件 */
...
/* 程序设置页：外框内 textarea/roller 透明铺满，高度由外框决定 */
...
/* 单行 textarea：铺满外框，one_line 后还原高度 */
static void program_admin_style_ta(lv_obj_t * ta)
{
    program_admin_style_field_inner(ta);
    lv_textarea_set_one_line(ta, true);
    lv_obj_set_height(ta, LV_PCT(100)); /* one_line 会改为 CONTENT，需还原外框高度 */
}
...
/* 离开声音控制子页：回管理员 8 宫格菜单 */
static void admin_sound_back_to_menu1(void)
{
    if(g_group_admin != NULL) {
        lv_group_set_editing(g_group_admin, false);
    }
...
static void program_admin_build_panel(lv_obj_t * root, lv_coord_t body_y, lv_coord_t body_h)
{
    ...
    lv_obj_center(g_admin_prog_total_val_lbl);

    for(int i = 0; i < TOTAL_PROGRAMS; i++) {
        g_admin_prog_btns[i] = make_admin_prog_btn(g_admin_panel_program, ui_translation(g_mode_name_ids[i]));
    ...
    for(int i = 0; i < 8; i++) {
        ...
        g_admin_prog_field_lbl[i] = lv_label_create(g_admin_panel_program);
        ui_lang_bind_label(g_admin_prog_field_lbl[i], field_ids[i]);
        ui_set_obj_font(g_admin_prog_field_lbl[i], s_font_sc_30);
        lv_obj_set_style_text_color(g_admin_prog_field_lbl[i], lv_color_hex(COL_TEXT), LV_PART_MAIN);
        lv_obj_set_width(g_admin_prog_field_lbl[i], field_w);
        lv_obj_set_style_text_align(g_admin_prog_field_lbl[i], LV_TEXT_ALIGN_CENTER, LV_PART_MAIN);
        lv_obj_set_pos(g_admin_prog_field_lbl[i], fx, field_y_lbl);
        ...
        lv_obj_add_flag(g_admin_prog_field_box[i], LV_OBJ_FLAG_OVERFLOW_VISIBLE); /* 编码器描边不被外框裁切 */
        ...
        lv_obj_center(g_admin_prog_dash[i]);
    ...
    lv_obj_center(g_admin_prog_water_lbl);
...
/* 校验 6 位数字机器 ID 并写入 g_machine_id */
...
//程序设置子面板：程序列表/程序时长/程序参数值/重置/确认
//屏幕亮度子面板：屏幕亮度标题/说明两行/常亮开关/亮度滑条（渐变外框）
//声音控制子面板：声音控制标题/触控声音与声音播报两行开关/音量与触控声音渐变滑条
//语言设置子面板：中英文切换/中文按钮/英文按钮
//待机时间子面板：当前待机时间/滚动条/确认按钮
//恢复默认子面板：恢复默认标题/恢复默认内容/恢复默认按钮/取消按钮
//联系我们子面板：联系我们标题/联系我们内容/小鸭二维码
//自投功能子面板：自投功能标题/说明两行/开启按钮/关闭按钮
//臭氧功能子面板：臭氧功能标题/说明两行/开启按钮/关闭按钮
//新风护理子面板：新风护理标题/说明两行/开启按钮/关闭按钮
//系统升级子面板：系统升级标题/确认文案/确认按钮/升级状态
//网络设置子面板：网络设置标题/WIFI设置与4G设置按钮
//WIFI 设置子面板：WIFI 设置标题（占位）
//4G 设置子面板：4G 设置标题/说明/开关/配网状态/成功图标
//数据设置子面板：数据设置标题/上传项复选框/上传策略互斥复选框
//支付设置子面板：支付设置标题/支付方式复选框/支付超时滚动条/订单查询提示与按钮
//密码修改子面板：密码修改标题/
...
static void build_pay(void)
{
        ...
        lv_obj_add_event_cb(btn_runpause, cb_load_screen, LV_EVENT_CLICKED, g_scr_pay_done); /* 模拟支付完成 */
        add_encoder_top_btn(top, "电源", 180, g_group_pay);
        ...
        lv_obj_t * img_qr = lv_image_create(center);      /* 程序对应二维码，非文字 */
        g_pay_qr_img = img_qr;
        lv_obj_set_size(img_qr, 250, 250);
        lv_image_set_inner_align(img_qr, LV_IMAGE_ALIGN_STRETCH);

        /* 金额 label：如「¥ 8.00」，由 pay_sync_price_label() 按程序填写，不参与语言表 */
        ...
        /* 扫码提示 label：中两行扫码说明 / 英 Scan the QR code…（STR_PAY_HINT） */
        g_lbl_pay_hint = lv_label_create(center);
        lv_obj_set_style_text_color(g_lbl_pay_hint, lv_color_hex(COL_TEXT), LV_PART_MAIN);
        lv_label_set_long_mode(g_lbl_pay_hint, LV_LABEL_LONG_WRAP);  /* 支持换行 \n */
        lv_obj_set_width(g_lbl_pay_hint, 560);                       /* 限制宽度自动折行 */
        lv_obj_set_style_text_align(g_lbl_pay_hint, LV_TEXT_ALIGN_CENTER, LV_PART_MAIN);
        ui_set_obj_font(g_lbl_pay_hint, s_font_sc_30);
        ui_lang_bind_label(g_lbl_pay_hint, STR_PAY_HINT);
        pay_sync_qr_image();                               /* 按 g_wheel_sel 设置 QR 图 */
        pay_sync_price_label();                            /* 按程序写金额文字 */
}
...
static void build_pay_done(void)
{
        ...
        add_encoder_top_btn(top, "启停", 100, g_group_pay_done);
        add_encoder_top_btn(top, "电源", 180, g_group_pay_done);
...
void ui_init(void)
{
        ...
        lv_group_set_wrap(g_group_home, false);//焦点不首尾循环
...
/* PC 仿真主循环调用；ui.c 为直接切屏，无 FSM 异步加载 */
void ui_tick(void)
{
}
