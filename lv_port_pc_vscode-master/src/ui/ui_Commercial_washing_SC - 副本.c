...
static bool pay_done_complete = false;
static bool need_scr_load = false;
uint16_t param_change[8];

bool* get_pay_done_complete() {return &pay_done_complete;}
...
static void admin_payment_sync_list_ui(void);  //支付设置页：刷新首页三项文案
static void admin_data_sync_upload_items_ui(void);  //数据设置页：刷新上传项开关
static void admin_data_sync_strategy_ui(void);  //数据设置页：刷新上传策略互斥开关
...
#define COL_SETTING_HINT    0xC6C4C4  //设置页说明小字灰（待机时间等）
...
static lv_group_t* g_group_home;   /* #16：仅主页轮播绑定编码器；其它页一律 NULL */
...
static lv_obj_t * g_admin_dormancy_tp_wrap;
static lv_obj_t * g_admin_dormancy_tp_lbl_prefix;
static lv_obj_t * g_admin_dormancy_tp_lbl_suffix;
static lv_obj_t * g_admin_dormancy_tp_digit_imgs[4];
static lv_obj_t * g_admin_dormancy_tp_digit_rollers[4];
static lv_obj_t * g_admin_dormancy_tp_lbl_colon;
static lv_obj_t * g_admin_dormancy_tp_btn_ok;
static lv_obj_t * g_admin_dormancy_tp_btn_cancel;
static uint32_t g_admin_dormancy_tp_minutes;
static uint32_t g_admin_dormancy_tp_seconds;
static bool g_admin_dormancy_tp_active = false;
/* 屏幕亮度子页控件（title_box + set_box；主文字+开关+说明+亮度样式滑条） */
...
static lv_obj_t * g_admin_brightness_fill;          /* 进度裁剪窗：宽度随数值变 */
static lv_obj_t * g_admin_brightness_fill_grad;     /* 整轨宽黑→蓝渐变 */
static lv_obj_t * g_admin_slider_brightness;
static lv_grad_dsc_t s_bright_page_fill_grad;       /* 亮度页整轨四段固定渐变 */
/* 声音控制子页控件（title_box + set_box；两项开关 + 亮度样式滑条） */
...
/* 臭氧功能子页控件（title_box + set_box；右上角开关） */
...
/* 新风护理子页控件（title_box + set_box；右上角开关） */
...
static lv_obj_t * g_admin_payment_order_summary_view;
...
static uint32_t g_admin_payment_tp_minutes;
static uint32_t g_admin_payment_tp_seconds;
...
/* admin_button_box 画布；点击区与可见底图对齐（#17 对齐 PC） */
...
static lv_font_t s_font_admin_kb;              /* SC_35 副本 + Montserrat 回退（图标键）键盘使用图标 */
static const lv_font_t * s_font_admin_kb_ptr;

static void admin_kb_font_init(void);  //初始化管理员数字键盘字体（中文 + Montserrat 回退）
static void cb_admin_kb_btn(lv_event_t * e);  //自定义数字键盘按键：数字/删除/确认
...
static void cb_admin_machine_confirm(lv_event_t * e);  //机器 ID 确认：保存并显示成功覆盖层
...
static void admin_brightness_apply_switch_layout(void);  //屏幕亮度页：开关样式
static void admin_panel_style_switch(lv_obj_t * sw);  //管理员子页开关 OFF 白 / ON 橙样式
static void admin_data_style_switch(lv_obj_t * sw);  //支付/待机等页：统一开关尺寸样式
...
static int32_t admin_brightness_snap_slider(int32_t v);  //亮度滑条：对齐到步进
...
static void admin_sound_volume_slider_sync_bar(int32_t v);  //同步音量填充宽度
static void admin_sound_touch_sound_volume_slider_sync_bar(int32_t v);  //同步触控声音填充宽度
...
static void cb_admin_auto_dose_btn_clicked(lv_event_t * e);  //用量按钮点击
static void admin_ozone_apply_switch_layout(void);  //臭氧开关布局（与 WIFI 一致）
...
static void admin_fresh_air_apply_switch_layout(void);  //新风开关布局（与 WIFI 一致）
...
static void admin_4g_set_phase(admin_4g_phase_t phase);  //4G 设置子页：切换说明/配网中/成功/失败 UI
...
static void cb_admin_4g_timer(lv_timer_t * t);  //2s 定时器：配网中 → 成功/失败态
static void cb_admin_4g_result_click(lv_event_t * e);  //4G 结果页点击
static void cb_admin_open_network_settings(lv_event_t * e);  //menu「网络设置」入口
...
static void ui_idle_apply_dormancy_period(void);  //按 g_ui_dormancy_timeout_ms / no_sleep 刷新空闲定时器
...
static lv_obj_t * g_running_child_lock_img;    /* 童锁图标（child_lock_logo） */
...
static lv_obj_t * g_end_btn_runpause;            /* 结束页顶栏启停 */
static lv_obj_t * g_end_btn_power;               /* 结束页顶栏电源 */
static lv_obj_t * g_pay_btn_runpause;            /* 支付页顶栏启停 */
static lv_obj_t * g_pay_btn_power;               /* 支付页顶栏电源 */
static lv_obj_t * g_pay_done_btn_runpause;       /* 支付完成页顶栏启停 */
static lv_obj_t * g_pay_done_btn_power;          /* 支付完成页顶栏电源 */
static lv_obj_t * g_home_btn_admin;              /* 主页底栏管理员入口 */
static lv_obj_t * g_home_carousel_enc;           /* 轮播区编码器焦点代理（旋转选程序，短按进支付） */
...
static void cb_home_runpause(lv_event_t * e);  //主页启停：触摸点击进支付
static void cb_home_prog_dot_focus(lv_event_t * e);  //轮播指示点：触摸点击带动画切程序
...
static void running_countdown_arm(void);  //若剩余时间>0 则启动秒倒计时定时器
...
static void running_live_hw_poll(void);  //实机：从 holding_register 拉频率/温度并刷新
...
static lv_obj_t * hw_active_runpause_btn(void)
{
	...
	if(scr == g_scr_selfcheck) return g_selfcheck_btn_runpause;
	if(scr == g_scr_cycle) return g_cycle_btn_runpause;
	return NULL;
}
...
static lv_obj_t * hw_active_power_btn(void)
{
	...
	if(scr == g_scr_selfcheck) return g_selfcheck_btn_power;
	if(scr == g_scr_cycle) return g_cycle_btn_power;
	return NULL;
}
...
static void ui_fsm_runpause_apply(bool long_press)
{
	...
	bool *is_payment_success = get_is_payment_success();

	if(*state == FSM_STANDBY) {
		*state = FSM_RUNNING;
	} else if(*state == FSM_RUNNING) {
		if(*is_payment_success) {
			*state = FSM_PAUSED;
		} else {
			*state = FSM_STANDBY;
		}
	} else if(*state == FSM_PAUSED) {
		*state = FSM_RUNNING;
	} else if(*state == FSM_OFF) {
		*state = FSM_STANDBY;
	}
...
/* 管理员数字键盘：数字键 ui_font_SC_35，缺字（图标等）回退 lv_font_montserrat_30 */
...
/* 顶部栏右侧：4G / WiFi / 时间（对齐 PC：wifi_logo + sc_27） */
static void create_top_status_bar(lv_obj_t * top, lv_obj_t ** clock_lbl_out)  //创建顶部右侧状态栏：4G / WiFi / 时间
{
...
/* 主页编码器：仅轮播代理；常驻 editing，旋转直接切程序（对齐 PC） */
...
/* 轮播编码器：旋转选程序；短按等同启停 → 进支付（对齐 PC；空白区已去 CLICKABLE） */
...
//将编码器输入设备绑定到指定 focus group（NULL 清除；不含侧键 keypad，侧键有独立 group）
...
static void running_child_lock_on_enter(void)  //童锁激活时的进入钩子（预留扩展）
{
	bool *child_lock_need_change = get_child_lock_need_change();
	if(child_lock_need_change == NULL) return;
	*child_lock_need_change = true;
}
...
//应用童锁锁定/解锁 UI（不操作编码器 group，对齐 PC）
...
//离开运行页时静默解除童锁
static void running_child_lock_remote_clear_if_leaving_running(lv_obj_t * target_scr)
{
	...
	running_child_lock_apply_locked(false, true);
}

//童锁按钮长按：切换童锁开/关
static void cb_running_child_lock_long(lv_event_t * e)
{
...
static void build_home(void)
{
		...
		/* --- 语言列：静态 language 图（对齐 PC；切语言在管理员页） --- */
		...
		lv_obj_remove_flag(g_home_carousel_enc, LV_OBJ_FLAG_CLICKABLE); /* 空白区不可点，防误进支付 */
		...
		lv_obj_remove_flag(lbl_manager, LV_OBJ_FLAG_CLICKABLE); /* 点击由下方扩大热区承接 */
		...
		g_home_btn_admin = lv_imgbtn_create(icon_row);               /* 第 5 列：管理员入口（视觉） */
        ...
        /* 顶栏置于轮播之上，使启停/电源的 ext_click 下扩不被 mid 抢走 */
...
/* 将剩余秒数格式化为 0:MM（对齐 PC：整分钟向上取整；有余秒时末档并入最后一分钟） */
...
#define RUNNING_COUNTDOWN_PERIOD_MS  1000u  /* 运行页倒计时：每秒递减 */
...
/* ---------- 实机硬件拉式读取（holding_register，与 get_exception_msg 同属硬件消息通道） ---------- */
/* 水位频率：百倍频，如 3920 → 39.20 KHz */
static uint16_t hw_running_freq_x100_get(void)
{
	return holding_register[MB_HOLD_WATER_LEVEL_FREQ_RO];
}

/* 水温：从保持寄存器高低字节合成 ℃ */
static int16_t hw_running_temp_c_get(void)
{
	return -(((holding_register[MB_HOLD_WATER_TEMP_RO] & 0xFF00) >> 8)
		- (holding_register[MB_HOLD_WATER_TEMP_RO] & 0x00FF));
}

/* 运行页：每帧从硬件拉频率/温度（暂停时也更新） */
static void running_live_hw_poll(void)
{
	if(g_scr_running == NULL || lv_scr_act() != g_scr_running) return;

	uint16_t freq = hw_running_freq_x100_get();
	int16_t temp_c = hw_running_temp_c_get();
	if(freq == g_running_freq_x100 && temp_c == g_running_live_temp_c) return;

	g_running_freq_x100 = freq;
...
static void build_running(void)
{
		...
		/* 勿与 cb_running_runpause 叠绑：会 RUNNING↔PAUSED 切两次，暂停闪烁被立刻恢复 */
...
static void build_end(void)
{
		...
		g_end_btn_runpause = add_top_text_btn(top, "启停", 100);
		...
		g_end_btn_power = add_top_text_btn(top, "电源", 180);
		...
		LV_IMAGE_DECLARE(success);                         /* 结束图标（对齐 PC） */
...
static ui_alarm_fault_ui_t g_alarm_fault_ui[UI_ALARM_FAULT_COUNT]; //E1..E15 UI 槽位
static ui_alarm_trigger_t g_alarm_trig_fault[UI_ALARM_FAULT_COUNT]; //E1..E15 触发记录
...
//读取第 idx 路故障是否活跃（0=E1；读 get_exception_msg()->exception_type）
static bool ui_fault_ex_active(uint8_t fault_idx)
{
	if(fault_idx >= UI_ALARM_FAULT_COUNT) return false; //越界保护
	ExceptionMsg* msg = get_exception_msg();
	if(msg == NULL) return false;
	return (bool)(msg->exception_type & (0x0002 << (fault_idx)));
}
...
//同步 15 路故障 + 2 路缺液的边沿状态
static void ui_alarm_trigger_sync(void)
{
	uint32_t seed = 0u;                                                    //同 tick 顺序偏移
	for(uint8_t i = 0; i < UI_ALARM_FAULT_COUNT; i++) {                    //遍历 E1..E15
...
{
	for(uint8_t i = 0; i < UI_ALARM_FAULT_COUNT; i++) {                    //隐藏 E1..E15
...
//显示报警弹层：显示当前轮播项、启动 3 秒定时器（不绑报警编码器组，对齐 PC）
...
{
	for(uint8_t i = 0; i < UI_ALARM_FAULT_COUNT; i++) {                    //E1..E15
		alarm_fault_panel_relayout_content(i);
	}
}

//构建 lv_layer_top() 报警全屏弹层：顶栏 + E1..E15 + 缺液 + 底栏
static void build_alarm_overlay(void)
{
	...
	lv_obj_t * img_alarm_back = lv_image_create(g_alarm_btn_back);
	lv_image_set_src(img_alarm_back, &back);
	lv_obj_center(img_alarm_back);
...
/* 写入表2.4 程序时限表初值到 g_prog_cfg_factory[]（对齐 PC：水位档 + 筒自洁秒级脱水）。 */
...
/*
 * 支付页 → MCU 参数表（读已确认 cfg；对齐 PC program_admin_fill_param_change）
...
/* 构建管理员菜单按钮（admin_button_box 底图 + 可选图标 + 文字） */
...
static void admin_panel_show(admin_view_t view)
{
    ...
    if(g_admin_mid_success_overlay != NULL) lv_obj_add_flag(g_admin_mid_success_overlay, LV_OBJ_FLAG_HIDDEN);
    ...
    /* #16：管理员页不绑定编码器 indev（对齐 PC；侧键 keypad 仍独立） */
    ui_set_encoder_group(NULL);
}
...
{
    admin_panel_show(MENU2); /* #17：亮度在 menu2 */
}
...
static void cb_admin_sound_slider_size_changed(lv_event_t * e)
{
    ...
    if(t == g_admin_slider_sound_volume) {
        admin_sound_volume_slider_sync_bar(0);
    }
    else if(t == g_admin_slider_touch_sound_volume) {
...
{
    (void)e;
    /* 暂关闭：不进入厂商维护（对齐 PC 不挂菜单）；自检/循环代码保留 */
}
...
{
    admin_panel_show(MENU2); /* #17：语言在 menu2 */
}
...
{
    admin_factory_timer_stop();
    admin_panel_show(MENU2); /* #17：恢复默认在 menu2 */
}
...
{
    admin_panel_show(MENU2); /* #17：联系我们在 menu2 */
}
...
{
    admin_panel_show(MENU1); /* #17：自投在 menu1 */
}
...
{
    admin_panel_show(MENU1); /* 臭氧在 menu1 */
}
...
{
    admin_panel_show(MENU1); /* 新风在 menu1 */
}
...
/* 待机/支付已改为 MM:SS 数字滚轮 + i18n bind，无需整表刷新 options */
...
/* 4G 开关 OFF→ON：进入配网中 UI，2s 后显示结果 */
static void admin_4g_start_provisioning(void)
{
    admin_4g_timer_stop();
    g_ui_4g_connect_result = 1; /* PC/仿真默认成功；真机由硬件写入 */
...
{
    admin_panel_show(MENU1); /* #17：网络在 menu1 */
}
...
{
    admin_wifi_timer_stop();
    g_ui_wifi_connect_result = 1; /* PC/仿真默认成功；真机由硬件写入 */
...
/* 数据设置页：按 g_ui_data_upload_* 刷新左栏上传项开关与文案 */
...
static void admin_data_back_to_menu1(void)
{
    ...
    admin_panel_show(MENU1); /* #17：数据在 menu1 */
}
...
static void build_admin(void)
{
    ...
    /* 密码子面板（PC 登录 chrome：标题 + title_bar + 提示 + input_box） */
    ...
    /* 自投功能子面板（title_box + set_box + 柔顺剂/洗衣液开关与用量按钮） */
    ...
    const lv_coord_t auto_row1_y  = 85;
    const lv_coord_t auto_sep_y   = 220;
    const lv_coord_t auto_row2_y  = 240;
    const lv_coord_t auto_row_pad = -15;
    const lv_coord_t auto_title_h = 48;
    const lv_coord_t auto_logo_trim0 = 34;
    const lv_coord_t auto_softener_scale_pct = 115;
    const lv_coord_t auto_softener_logo_w = (96 * auto_softener_scale_pct + 50) / 100;
    const lv_coord_t auto_softener_logo_h = (102 * auto_softener_scale_pct + 50) / 100;
    const lv_coord_t auto_softener_trim = (auto_logo_trim0 * auto_softener_scale_pct + 50) / 100;
    const lv_coord_t auto_softener_content_w = (28 * auto_softener_scale_pct + 50) / 100;
    const lv_coord_t auto_detergent_trim = auto_logo_trim0;
    const lv_coord_t auto_detergent_content_w = 30;
    const lv_coord_t auto_icon_text_gap = 10;
    const lv_coord_t auto_dose_btn_w = 153;
    ...
    lv_obj_set_size(img_softener_logo, auto_softener_logo_w, auto_softener_logo_h);
    ...
    for(int i = 0; i < 4; i++) {
        ...
        lv_image_set_src(bg, &auto_put_btn_box);
        lv_obj_center(bg);
    ...
    /* 数据设置子面板（title_box + set_box；列表→策略两页） */
    ...
    lv_obj_set_style_text_letter_space(g_admin_ta_machine_id, 8, LV_PART_MAIN);
    ...
    lv_obj_t * img_mid_success = lv_image_create(g_admin_mid_success_overlay);
    lv_image_set_src(img_mid_success, &success);
    lv_obj_align(img_mid_success, LV_ALIGN_CENTER, 0, -80);
    lv_obj_add_flag(img_mid_success, LV_OBJ_FLAG_EVENT_BUBBLE);

    lv_obj_t * lbl_mid_success = lv_label_create(g_admin_mid_success_overlay);
    ui_lang_bind_label(lbl_mid_success, STR_MACHINE_ID_SUCCESS);
    lv_obj_set_style_text_color(lbl_mid_success, lv_color_hex(COL_TEXT), LV_PART_MAIN);
    ui_set_obj_font(lbl_mid_success, s_font_sc_50);
    lv_obj_align(lbl_mid_success, LV_ALIGN_CENTER, 0, 60);
    lv_obj_add_flag(lbl_mid_success, LV_OBJ_FLAG_EVENT_BUBBLE);
...
static void build_pay(void)
{
		...
		g_pay_btn_runpause = add_top_text_btn(top, "启停", 100);
		...
		g_pay_btn_power = add_top_text_btn(top, "电源", 180);
...
static void build_pay_done(void)
{
		...
		g_pay_done_btn_runpause = add_top_text_btn(top, "启停", 100);
		...
		g_pay_done_btn_power = add_top_text_btn(top, "电源", 180);
		...
		LV_IMAGE_DECLARE(success);
		lv_obj_t * img_done = lv_image_create(center);      /* 支付完成用 success 图标（对齐 PC） */
...
static void cycle_enter_fault_display(void)
{
	...
	{
		uint8_t *st = get_fsm_state();
		if(st != NULL) *st = FSM_STANDBY;
	}
...
/* ── check.c 所需 UI getter/wrapper ── */

int ui_selfcheck_state_get(void)
{
	return (int)g_selfcheck_ui_state;
}

void ui_selfcheck_state_set(int state)
{
	selfcheck_ui_state_t step = (selfcheck_ui_state_t)state;
	selfcheck_ui_goto_step(step);
}

bool ui_cycle_active_get(void)
{
	return g_cycle_active;
}

int ui_cycle_ui_state_get(void)
{
	return (int)g_cycle_ui_state;
}

void ui_cycle_session_set_fault_code(uint8_t code)
{
	g_cycle_session.fault_code = code;
}

void ui_cycle_session_set_fault_at_count(uint32_t count)
{
	g_cycle_session.fault_at_count = count;
}

void ui_cycle_enter_fault_display(void)
{
	cycle_enter_fault_display();
}

void ui_cycle_finish_current_run(void)
{
	cycle_finish_current_run();
}

void ui_cycle_start_run(void)
{
	cycle_start_run();
}
...
static void build_off(void)
{
		...
		g_off_btn_runpause = add_top_text_btn(top, "启停", 100);
...
static lv_obj_t* state_to_scr(uint8_t state)
{
	...
	switch(state) {
		...
		case FSM_STANDBY: {
			bool* is_first_standby = get_is_first_standby();
			if(*is_first_standby) {
				scr = g_scr_off;
			} else {
				scr = g_scr_home;
		...
		case FSM_RUNNING: {
			...
			if(!*is_payment_success) {
				scr = g_scr_pay;
			} else if (*is_payment_success && !pay_done_complete) {
				scr = g_scr_pay_done;
			} else {
				scr = NULL;
			}
...
void ui_init(void)
{
		...
		g_group_home = lv_group_create();                   /* #16：仅主页编码器 focus 组 */
		lv_group_set_wrap(g_group_home, false);
...
{
	uint8_t* state;
	TickType_t last_flush_tick = 0;
	bool display_comm_armed = false;
	TickType_t display_comm_no_flush_since = 0;
	bool display_comm_invalidated = false;
...
#define DISPLAY_COMM_FAULT_TICKS  pdMS_TO_TICKS((uint32_t)LV_DEF_REFR_PERIOD * 4u)
	...
	while (1) {
		...
		running_live_hw_poll();            /* 运行页水位频率/温度：拉 holding_register */
		...
		state = get_fsm_state();
		if(ui_dormancy_inactive_should_off() && *state == FSM_STANDBY) {
			*state = FSM_OFF;
		}

		vTaskDelay(pdMS_TO_TICKS(delay_ms));

		/* ── 显示屏通信心跳检测（时间阈值基于 LV_DEF_REFR_PERIOD） ── */
		...
		if (!display_comm_armed) {
			...
			if (now != last_flush_tick) {
				...
				if (elapsed >= DISPLAY_COMM_FAULT_TICKS) {
					rtos *app = get_rtos();
					ExceptionMsg em = { .exception_type = EXCEPTION_DISPLAY_COMM };
					xQueueSend(app->q_exception, &em, pdMS_TO_TICKS(100));
					display_comm_no_flush_since = xTaskGetTickCount();
					display_comm_invalidated = false;
				}
			}
