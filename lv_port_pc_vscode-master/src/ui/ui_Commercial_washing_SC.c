 ...
 * 0：实机：主循环 running_live_hw_poll() 拉硬件*/
#define UI_RUNNING_LIVE_PC_SIM  0
...
static void running_live_hw_poll(void);  //实机：从硬件占位接口拉频率/温度并刷新
...
static void ui_screen_load(lv_obj_t * scr)
{
	...
	if(scr == g_scr_off) {
		...
		param_change[0] = g_prog_cfg[g_wheel_sel].wash_min * 60;
		param_change[1] = g_prog_cfg[g_wheel_sel].rinse_min * 60;
		...
		param_change[5] = g_prog_cfg[g_wheel_sel].spin_min * 60;
...
static void build_home(void)
{
        ...
        lv_obj_set_ext_click_area(g_btn_home_lang, 16);
        ...
        lv_obj_set_ext_click_area(g_home_btn_admin, 16);
...
/* ---------- 实机硬件拉式读取（占位，对齐 get_exception_msg 用法） ---------- */
/*
 * TODO: 换成真实寄存器/消息接口，例如：
 *   SensorMsg *s = get_sensor_msg();
 *   return s->water_freq_x100;  // 百倍频，如 3920 → 39.20 KHz
 */
 ...
 * TODO: 换成真实寄存器/消息接口，例如：
 *   SensorMsg *s = get_sensor_msg();
 *   return (int16_t)s->water_temp_c;  // 单位 ℃
 */
...
/* 运行页：每帧从硬件拉频率/温度（暂停时也更新）；PC 仿真时跳过 */
...
#define DISPLAY_COMM_FAULT_TICKS  pdMS_TO_TICKS((uint32_t)LV_DEF_REFR_PERIOD * 4u)
	...
	while (1) {
		...
		running_live_hw_poll();            /* 运行页水位频率/温度：拉硬件占位接口 */
		...
		if(inactive_time >= 300000 && *state == FSM_STANDBY) {
			*state = FSM_OFF;
		}
